...
document.addEventListener("DOMContentLoaded", function () {
    ...
    passwordReg.addEventListener("change", function () {
        ...
        if (!checkPassStrength(this.value)) {
            showLoginError("Password must contain at least 8 characters, including uppercase, lowercase, and numbers");
        }
    });
        ...
        .then(data => {
            switch (data.message) {
                ...
                case 'invalid_password':
                    showLoginError(`Invalid password. ${data.remainingAttempts} attempts remaining.`);
                    break;

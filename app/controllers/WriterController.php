...
class WriterController extends Controller
{
    ...
    public function viewSubmission($submissionID = 0)
    {
        ...
        $userDetails = $user->first(['userID' => $submissionDetails['userID']]);
    ...
    public function Update()
    {
        ...
        if (isset($_FILES['cover_image']) && $_FILES['cover_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $uploadPath)) {
                ...
                $coverImageID = $coverImageDetails['covID'] ?? null;

                if (!$coverImageID) {
                    echo "Failed to retrieve cover image ID.";
                    exit;
                }
            } else {
        ...
        if (!$coverImageID) {
            ...
            $coverImageID = $existingBook['coverImage'] ?? null;
        }

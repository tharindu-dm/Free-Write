      ...
      <tbody>
        <?php if (!empty($data['processedOrderDetails'])): ?>
          <?php foreach ($data['processedOrderDetails'] as $order): ?>
            ...
            ?>
            <tr>
              <td><a href="/Free-Write/public/Order/orderDetail/<?= htmlspecialchars($order['orderID'])?>/<?= htmlspecialchars($order['customer_userID'])?>"><?= htmlspecialchars($order['bookTitle'] ?? ''); ?></a></td>

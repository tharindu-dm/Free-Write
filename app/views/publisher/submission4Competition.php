...
<head>
    ...
    <style>
        ...
        .modal {
            ...
            z-index: 1000; /* Ensure modal is above header */
        }
...
?>
    ...
    <main class="container">
        ...
        <form id="submission-form" method="POST" action="/Free-Write/public/Competition/SubmitEntry">
            <input type="hidden" name="competition_id" value="<?= htmlspecialchars($data['competition']['competitionID'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <textarea id="submission-description" name="description" required placeholder="Please fill in the field"></textarea>
                    ...
                    <select id="book-select" name="selected_book">
                        ...
                        <?php if (!empty($data['books'])): ?>
                            <?php foreach ($data['books'] as $book): ?>
                                <option value="<?= htmlspecialchars($book['bookID']) ?>"><?= htmlspecialchars($book['title']) ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                            ...
                            <?php if (!empty($data['books'])): ?>
                                <?php foreach ($data['books'] as $book): ?>
                                    <div class="book-preview-item">
                                        <img src="<?= !empty($book['coverImage']) ? '/Free-Write/public/uploads/covers/' . htmlspecialchars($book['coverImage']) : '/Free-Write/public/images/collectionThumb.jpeg' ?>" alt="Book cover">
                                        <div><?= htmlspecialchars($book['title']) ?></div>
                                        <button type="button" class="select-book-btn" data-id="<?= htmlspecialchars($book['bookID']) ?>" data-title="<?= htmlspecialchars($book['title']) ?>">Select</button>
                                    </div>
                                <?php endforeach; ?>
                            <?php else: ?>
